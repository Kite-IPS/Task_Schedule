...
const Table = ({ data = [], onView }) => {
  ...
  const uniqueFollowUps = [...new Set(safeData.map((item) => item.followUp).filter(Boolean))];
  ...
  const filteredData = useMemo(() => {
    return safeData.filter((item) => {
      ...
      return (
        ...
        (filters.followUp === "" || item.followUp === filters.followUp)
      );
    });
  }, [safeData, filters]);
              ...
              <div>
                ...
                >
                  ...
                  {uniqueFollowUps.map((followUp) => (
                    <option key={followUp} value={followUp} className="bg-gray-900">
                      {followUp}
                    </option>
                  ))}
                </select>
